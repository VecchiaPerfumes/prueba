...
(function () {
      ...
      if (isCart) {
        ...
        body.innerHTML = lines.map(function (l) {
          ...
          return '<div class="line" data-line="' + esc(p.id) + '">' +
            '<div class="line__img">' + imgTag(p, 'lazy') + '</div>' +
            '<div>' +
      ...
      box.innerHTML = '<p class="meta" style="color:var(--paper-45);padding:.75rem 0">' +
        ...
        res.map(function (p) {
          return '<a class="result" href="producto.html?id=' + esc(p.id) + '">' +
            '<span class="result__img">' + imgTag(p, 'lazy') + '</span>' +
  ...
  function cardHTML(p, opts) {
    ...
    return '<article class="card reveal" data-id="' + esc(p.id) + '">' +
      '<a class="card__media" href="producto.html?id=' + esc(p.id) + '" ' +
  ...
  window.Vecchia = {
    ...
    imgTag: imgTag, observeReveals: observeReveals, ICON: ICON, norm: norm
  };
